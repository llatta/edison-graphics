...
static void lnw_gpio_set(struct gpio_chip *chip, unsigned offset, int value)
{
	...
	if (value) {
		//printk(KERN_DEBUG "My: lnw_gpio_set %p %u\n", gpsr, offset);
		if (offset == 165)
		{
			...
			gpsr = gpio_reg(chip, TFT_EDISON_DATA0, GPSR);
			gpcr = gpio_reg(chip, TFT_EDISON_DATA0, GPCR);

			printk(KERN_DEBUG "My: Custom flood nowritel %p %p %i %u %u\n", gpsr, gpcr, len, hi, lo);
